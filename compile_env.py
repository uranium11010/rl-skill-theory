"""
Adapted from CompILE grid world implementation by Jiang et al. 2022
https://github.com/yidingjiang/love/blob/main/grid_world/grid.py
"""
...
class CompILEEnv(GridEnv):
    ...
    def __init__(self, seed=1, max_steps=50, width=10, height=10, num_objects=6,
        ...
        self._initial_objects = [CompILEObject.random_object(random_state)
                                 for _ in range(num_objects)]

        self._object_positions = []
        for obj in self._initial_objects:
            while True:
                x = random_state.randint(1, width - 1)
                y = random_state.randint(1, height - 1)
                allowed = (x, y) not in self._object_positions
                if allowed:
                    self._object_positions.append((x, y))
                    break

        num_walls = random_state.randint(5)
        while True:
            ...
            for _ in range(num_walls):
                ...
                y = random_state.randint(1, height - 1)
                pos = (x, y)
                length = random_state.randint(1, 4)
                directions = [(1, 0), (0, 1), (-1, 0), (0, -1)]
                direction = directions[random_state.randint(len(directions))]
        ...
        self._object_types = [obj.type for obj in self._initial_objects]
        self._items_to_visit = random_state.choice(
                self._object_types, size=visit_length, replace=False)
    ...
    def _place_objects(self):
        ...
        for obj, position in zip(self._initial_objects, self._object_positions):
            self.place(obj, position, exist_ok=True)
    ...
    def reset(self, *, seed=None, options=None):
        ...
        while True:
            self._initial_agent_pos = (
                self.np_random.integers(1, self._width - 1),
                self.np_random.integers(1, self._height - 1))
            obj = self.get(self._initial_agent_pos)
            allowed = obj is None or isinstance(obj, CompILEObject)
            allowed = (
                    allowed and
    ...
    def step(self, action):
        ...
        if action == Action.pickup:
            obj = self.get(self._agent_pos)
            item_to_visit = self._items_to_visit[self._visited_index] if self._visited_index < len(self._items_to_visit) else None
        ...
        if self._visited_index == len(self._items_to_visit) and pickup:
            reward = 1
            done = True

        state, _, max_steps, info = super().step(action)

        return state, reward, done, max_steps, info
    ...
    def __str__(self):
        ...
        state_obs, item = self._gen_obs()
        out_str += pretty_print_state(state_obs)
        out_str += f"CURRENT GOAL: {item}\n"
        return out_str


def get_cell_string(cell_state):
    cell_state = list(cell_state)
    agent_here = False
    if cell_state[10]:
        agent_here = True
        cell_state[10] = 0
    agent_str = '@' if agent_here else ' '
    try:
        index = cell_state.index(1.)
        if index == 11:
            return '##'
        else:
            object_str = str(index)
    except:
        object_str = ' '
    return agent_str + object_str
